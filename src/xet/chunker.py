"""Gearhash-based Content-Defined Chunking (CDC).

Implements the same algorithm as xet-core: Gearhash rolling hash with
min/max chunk size constraints and a bitmask boundary condition.
"""
...
# fmt: off
# 256-entry Gearhash lookup table from rust-gearhash
# https://github.com/srijs/rust-gearhash/blob/master/src/table.rs
GEAR_TABLE: list[int] = [
...
def chunk_data(
    ...
    Algorithm matches xet-core:
    1. Skip hash computation for first (min_size - GEAR_WINDOW - 1) bytes
    2. Boundary when (hash & MASK) == 0
    3. Force boundary at max_size
    4. Remaining bytes at EOF become the final chunk
    """
    ...
    while start < total:
        ...
        if remaining <= min_size:
            # Last piece is smaller than min — emit as final chunk
            chunks.append(Chunk(offset=start, data=data[start:]))
            break

        h: int = 0
        # Optimization: skip hashing for first (min_size - GEAR_WINDOW - 1) bytes
        # because the Gear hash at min_size only depends on the last 64 bytes.
        skip = max(0, min_size - GEAR_WINDOW - 1)
        scan_start = start + skip
        # Pre-warm the hash with skipped bytes (still need to feed them)
        for i in range(start, scan_start):
            h = ((h << 1) + GEAR_TABLE[data[i]]) & U64_MAX
        ...
        if boundary == -1:
            # No boundary found — force at max_size (or EOF)
            boundary = end

        chunks.append(Chunk(offset=start, data=data[start:boundary]))
        start = boundary
